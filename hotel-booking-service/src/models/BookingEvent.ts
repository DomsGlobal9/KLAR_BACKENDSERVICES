...
export const BookingEventModel = mongoose.model<IBookingEvent>("BookingEvent", BookingEventSchema);

export class BookingEventLogger {
    static async log(bookingId: string, requestId: string, eventType: string, payload?: any) {
        try {
            console.log(`[EVENT] ${eventType} for Booking: ${bookingId} (Req: ${requestId})`);
