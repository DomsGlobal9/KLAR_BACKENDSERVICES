...
export class ValidationEngine {
    ...
    public static validate(
        ...
        if (!freshResult.available) {
            throw new StructuredError("ROOM_SOLD_OUT", "The requested room is no longer available.");
        }
        ...
        if (expectedResult) {
            if (expectedResult.roomType && freshResult.roomType) {
                ...
                const normExpected = expectedResult.roomType.replace(/[^a-z0-9]/gi, '').toLowerCase();
                const normFresh = freshResult.roomType.replace(/[^a-z0-9]/gi, '').toLowerCase();
                
                // Allow partial match (e.g. "Standard Room" in "Standard Room Double")
                if (!normFresh.includes(normExpected) && !normExpected.includes(normFresh)) {
                    console.warn(`[ValidationEngine] Room name mismatch. Expected: ${expectedResult.roomType}, Fresh: ${freshResult.roomType}`);
                }
            }

            if (expectedResult.mealPlan && freshResult.mealPlan) {
                const normExpected = expectedResult.mealPlan.replace(/[^a-z0-9]/gi, '').toLowerCase();
                const normFresh = freshResult.mealPlan.replace(/[^a-z0-9]/gi, '').toLowerCase();
                if (normExpected !== normFresh && !normFresh.includes(normExpected)) {
                    console.warn(`[ValidationEngine] Meal plan mismatch. Expected: ${expectedResult.mealPlan}, Fresh: ${freshResult.mealPlan}`);
                }
            }
            
            if (expectedResult.cancellationPolicyHash && freshResult.cancellationPolicyHash) {
                if (expectedResult.cancellationPolicyHash !== freshResult.cancellationPolicyHash) {
                    throw new StructuredError("POLICY_CHANGED", "Supplier has modified the cancellation policy.", {
        ...
        // Price comparison
        const totalFreshPrice = Number(freshResult.price || 0) + Number(freshResult.taxes || 0);

        if (expectedResult && expectedResult.price !== undefined) {
            const expectedPrice = Number(expectedResult.price);
            const tolerance = Math.max(toleranceConfig.fixed, expectedPrice * (toleranceConfig.percent / 100));

            if (totalFreshPrice > expectedPrice + tolerance) {
                throw new StructuredError("PRICE_CHANGED", "Supplier price has increased beyond allowed tolerance.", {
                    expectedPrice,
