...
export class BookingTemplateService {
  ...
  private compileHtml(target: "client" | "agent", booking: any): string {
    ...
    if (!["CONFIRMED", "CANCELLED", "HELD"].includes(rawStatus)) {
      throw new Error(
        `Invalid booking status: '${rawStatus}'. Templates can only be generated for CONFIRMED, CANCELLED, or HELD statuses.`,
      );
    }
    ...
    if (!fs.existsSync(templatePath)) {
      throw new Error(
        `Target HTML template file not found at path: ${templatePath}`,
      );
    }
    ...
    // Route 2: Fallback iteration path for customized cancellation flat database entries
      ...
      for (let i = 0; i < totalGuestsCount; i++) {
        ...
        const secondaryName =
          i === 0
            ? String(booking.guestName || "SUDHEER GANTA").toUpperCase()
            : `GUEST COMPANION ${i + 1}`;
        passengerRows += `
                    <tr>
    ...
    if (booking.guestMobile) {
      ...
      const countryCode =
        booking.tripJackRequest?.deliveryInfo?.code?.[0] || "";
      clientPhone =
        `${countryCode} ${booking.tripJackRequest.deliveryInfo.contacts[0]}`.trim();
    }
    ...
    if (
      booking.rooms?.[0]?.boardType &&
      booking.rooms[0].boardType.trim() !== ""
    ) {
    ...
    const cancellationPenaltyVal =
      booking.cancelCharge !== undefined
        ? booking.cancelCharge
        : booking.cancellationDetails?.penalties?.[0]?.amount || 0;
    ...
    html = html
      ...
      .replace(
        /{{hotelName}}/g,
        booking.hotelName || "Selected Hotel Portfolio",
      )
      ...
      .replace(
        /{{cancellationPenalty}}/g,
        Number(cancellationPenaltyVal).toLocaleString("en-IN", {
          minimumFractionDigits: 3,
          maximumFractionDigits: 3,
        }),
      )
      .replace(
        /{{refundAmount}}/g,
        Number(refundAmountVal).toLocaleString("en-IN", {
          minimumFractionDigits: 3,
          maximumFractionDigits: 3,
        }),
      )
      ...
      .replace(
        /{{provider}}/g,
        String(booking.provider || "tripjack").toUpperCase(),
      )
      ...
      .replace(
        /{{netAmount}}/g,
        Number(booking.netAmount || 0).toLocaleString("en-IN", {
          minimumFractionDigits: 3,
          maximumFractionDigits: 3,
        }),
      )
      .replace(
        /{{markupAmount}}/g,
        Number(booking.markupAmount || 0).toLocaleString("en-IN", {
          minimumFractionDigits: 3,
          maximumFractionDigits: 3,
        }),
      );
