...
class AmendService {
    async getModificationPolicy(confirmationNumber: string) {
        ...
        const rgPolicy = booking.rateGainResponse?.body?.modificationPolicies || 
    ...
    async getModificationPricing(payload: any) {
        ...
        return {
            ...
            allowed: true,
            estimatedCharges: 0,
            message: "Modification request received. Standard rates will apply for new dates."
        };
    }
    ...
    async commitModification(payload: any) {
        ...
        if (booking.provider === "tripjack") {
            ...
            return await tripJackProvider.amend({ bookingId: confirmationNumber, checkIn, checkOut });
        }
