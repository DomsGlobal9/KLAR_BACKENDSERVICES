...
class PrecheckService {
    async precheck(payload: any) {
        const propertyId: string = payload.propertyId || payload.PropertyId || payload.BookReservation?.propertyID || "";
        ...
        if (propertyId.startsWith("TJ:")) {
            ...
            console.log("[TripJack Precheck] Raw payload received:", JSON.stringify(payload, null, 2));
            console.log("[TripJack Precheck] Unwrapped payload to provider:", JSON.stringify(tjPayload, null, 2));
