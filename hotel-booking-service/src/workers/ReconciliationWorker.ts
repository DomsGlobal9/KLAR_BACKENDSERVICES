  ...
  private static async resolveStuckBooking(booking: any) {
    try {
      ...
      if (
        ...
        console.log(
          `[ReconciliationWorker] Refunding stuck manual review booking: ${booking.confirmationNumber}`,
        );
        await WalletUtil.refundBalance(
          token,
          booking.totalAmount,
          demandId,
          "Reconciliation Worker: Auto-refund for unresolved booking",
        );

        booking.status = BookingStatus.FAILED;
        booking.failureReason =
          "Auto-refunded by Reconciliation Worker due to prolonged MANUAL_REVIEW status.";
        await booking.save();
