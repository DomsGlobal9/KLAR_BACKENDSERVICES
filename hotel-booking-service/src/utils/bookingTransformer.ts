...
export const compileTravellerPayload = (formData: any, providerContext: any) => {
  ...
  if (isTJ) {
    ...
    console.log("[DEBUG] formData.RoomSelection:", JSON.stringify(formData.RoomSelection, null, 2));
    console.log("[DEBUG] formData.rooms:", JSON.stringify(formData.rooms, null, 2));
    ...
    return {
      ...
      BookReservation: {
        ...
        CurrencyCode: providerContext.currency || formData.CurrencyCode || formData.Currency || "USD",
        TimeStamp: new Date().toISOString(),
        ReservationDate: new Date().toISOString(),
        checkin: providerContext.checkIn || formData.checkIn || formData.checkin,
        checkout: providerContext.checkOut || formData.checkOut || formData.checkout,
        propertyID: providerContext.hotelId || formData.hotelId || formData.propertyID,
        PropertyCode: formData.PropertyCode || formData.propertyCode || providerContext.hotelId,
        BrandCode: formData.BrandCode || formData.brandCode || "N/A",
        EchoToken: formData.EchoToken || `echo-${Date.now()}`,
        Session: formData.Session || "",
        BookingRate: providerContext.totalAggregatePrice || formData.totalNet || formData.totalPrice,
        RoomSelection: formData.rooms.map((room: any) => ({
          RoomTypeCode: room.roomTypeCode || formData.RoomSelection?.[0]?.RoomTypeCode || "Standard",
          NumberOfRooms: 1,
          RoomSelectionKey: room.selectionKey || room.optionId || room.rateKey || formData.RoomSelection?.[0]?.RoomSelectionKey || "",
          RoomRate: room.rate || room.price || formData.RoomSelection?.[0]?.RoomRate || 0,
          BoardName: room.boardName || formData.RoomSelection?.[0]?.BoardName || "ROOM ONLY",
          Guest: room.guests.filter((g: any) => g.isAdult).map((g: any, idx: number) => {
            const frontendGuest = formData.RoomSelection?.[0]?.Guest?.[0] || {};
            return {
              FirstName: g.firstName || frontendGuest.FirstName || "Guest",
              LastName: (g.lastName || frontendGuest.LastName || "Guest") + (idx > 0 ? `Adult${idx + 1}` : ""),
              Primary: idx === 0,
              Email: g.email || formData.primaryEmail || formData.email || frontendGuest.Email || "[EMAIL_ADDRESS]",
              EmailType: 1,
              ProfileType: 1,
              Phone: g.mobile || formData.primaryPhone || formData.mobile || formData.phone || frontendGuest.Phone || "0000000000",
              CountryCode: formData.countryCodeISO || formData.CountryCode || frontendGuest.CountryCode || "IN",
              StateCode: formData.stateCode || frontendGuest.StateCode || "TN",
              City: formData.city || frontendGuest.City || "N/A",
              Line1: formData.addressLine1 || formData.hotelAddress || frontendGuest.Line1 || "N/A",
              PostalCode: formData.postalCode || frontendGuest.PostalCode || "500001"
            };
          }),
