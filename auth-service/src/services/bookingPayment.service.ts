...
import { BadRequestError, NotFoundError } from "../errors/AppError";
import { UserModel } from "../models/user.model";
import { EmailService } from "./email.service"; // Adjust this path to match your folder structure
...
export class BookingPaymentService {
    ...
    private static async handleRMBalanceCheck(
        ...
        // Trigger email if balance is insufficient
        if (!hasSufficientBalance) {
            EmailService.sendInsufficientBalanceEmail(
                [parentAdmin.email, user.email],
                {
                    userName: user.email,
                    requiredAmount: totalPrice,
                    currentBalance: parentWallet.balance,
            ...
            ).catch(err => console.error("[EMAIL ERROR] Failed to send insufficient balance email:", err.message));
        }
    ...
    private static async handleB2BAdminBalanceCheck(
        ...
        if (isSubCompany) {
            ...
            return await this.handleParentAdminBalanceCheck(userId, user, wallet, bookingId, totalPrice);
        }
    }
    ...
    private static async handleSubCompanyBalanceCheck(
        ...
        // Sub-company has insufficient balance, check parent wallet availability
        const hasParentSufficientBalance = parentWallet.balance >= totalPrice;
        const shortfallAmount = hasParentSufficientBalance ? 0 : totalPrice - parentWallet.balance;

        // Trigger email if BOTH child and parent fail the balance test
    ...
    private static async handleParentAdminBalanceCheck(
        ...
        // Trigger email if parent balance check fails
        if (!hasSufficientBalance) {
            EmailService.sendInsufficientBalanceEmail(
                user.email,
                {
    ...
    private static async handleRMPayment(
        ...
        // Trigger payment confirmation to parent admin and RM
        EmailService.sendPaymentDeductedEmail(
            [parentAdmin.email, user.email],
            {
                accountName: `Parent Admin Wallet (Debited by RM: ${user.email})`,
                amount: totalPrice,
                bookingId,
                newBalance: updatedParentWallet.balance ?? 0
            }
        ).catch(err => console.error("[EMAIL ERROR] Payment debit email failed:", err.message));
    ...
    private static async handleB2BAdminPayment(
        ...
        if (isSubCompany) {
            ...
            return await this.handleParentAdminPayment(userId, user, wallet, bookingId, totalPrice);
        }
    }
    ...
    private static async handleSubCompanyPayment(
        ...
        if (hasSufficientBalance) {
            ...
            // Trigger payment email to Child/Sub-company
            EmailService.sendPaymentDeductedEmail(
                user.email,
                {
                    accountName: `Sub-Company Wallet (${user.email})`,
                    amount: totalPrice,
                    bookingId,
                    newBalance: updatedSubCompanyWallet.balance ?? 0
                }
            ).catch(err => console.error("[EMAIL ERROR] Payment debit email failed:", err.message));
        ...
        // Sub-company has insufficient balance - deduct from parent, force child into debt
        console.log("[handleSubCompanyPayment] Sub-company has insufficient balance, deducting from parent only");
        console.log("[handleSubCompanyPayment] Parent wallet balance:", parentWallet.balance);
        ...
        // Deduct same amount from sub-company (making it more negative / debt representation)
        const updatedSubCompanyWallet = await BookingPaymentRepository.deductBalanceAllowNegative(
            subCompanyWallet._id,
            totalPrice
        );
        ...
        if (!updatedSubCompanyWallet) {
            // Rollback parent transaction if child update fails
            await BookingPaymentRepository.addBalance(parentWallet._id, totalPrice);
            throw new BadRequestError("Failed to deduct from sub-company wallet");
        }
        ...
        // Trigger Parent Deduction Confirmation
        EmailService.sendPaymentDeductedEmail(
            parentAdmin.email,
            {
                accountName: `Parent Wallet (Covering Child: ${user.email})`,
                amount: totalPrice,
                bookingId,
                newBalance: updatedParentWallet.balance ?? 0
            }
        ).catch(err => console.error("[EMAIL ERROR] Parent deduction email failed:", err.message));

        // Trigger Hierarchical Debt Alert (-amount credited from parent to child statement context)
        EmailService.sendChildNegativeDebtEmail(
            parentAdmin.email,
            user.email,
            {
                childName: user.email,
                amount: totalPrice,
                childNewBalance: updatedSubCompanyWallet.balance ?? 0
            }
        ).catch(err => console.error("[EMAIL ERROR] Debt alert email failed:", err.message));
    ...
    private static async handleParentAdminPayment(
        ...
        // Trigger payment confirmation email to Parent Admin
        EmailService.sendPaymentDeductedEmail(
            user.email,
            {
                accountName: `Parent Admin Wallet (${user.email})`,
                amount: totalPrice,
                bookingId,
