...
async function main() {
    try {
        ...
        const searchRes = await axios.post("http://localhost:5012/api/search/hotels/search", {
        ...
        const rgHotel = hotels.find((h: any) => h.hotelId && h.hotelId.startsWith("RG:"));

        if (!rgHotel) {
            console.error("No RateGain hotel found in search results. Found:", hotels.map((h: any) => h.hotelId || h.id));
            return;
        }
        ...
        const productsRes = await axios.post(`http://localhost:5012/api/search/hotels/${rgHotel.hotelId}/products`, {
            PropertyCode: rgHotel.propertyCode || rgHotel.hotelId.replace("RG:", ""),
        ...
        console.log("Raw RateGain products response structure keys:", Object.keys(body));
        ...
        if (products.length > 0) {
            ...
            if (rates.length > 0) {
                console.log("First rate object details:", JSON.stringify(rates[0], null, 2));
            } else {
                console.log("No rates found in first product:", firstProduct);
            }
        } else {
            console.log("No products found in response body:", JSON.stringify(body, null, 2).substring(0, 1000));
        }

    } catch (e: any) {
        console.error("Error running diagnostics:", e.response?.status, e.response?.data || e.message);
    }
}
