...
export class RateGainApiProvider {
    ...
    async getBestProperties(payload: any) {
        const rateGainPayload: any = {
            ...
            Rooms: (payload.Rooms || payload.rooms || []).map((r: any) => {
                ...
                return { NumberOfRoom: r.NumberOfRoom || r.numberOfRoom || 1, Adults: adultsCount, Children: childrenCount, paxes };
            }),
        ...
        if (payload.CountryCode || payload.countryCode) rateGainPayload.CountryCode = payload.CountryCode || payload.countryCode;
        if (payload.Currency || payload.currency) rateGainPayload.Currency = payload.Currency || payload.currency;
    ...
    async getAllProducts(payload: any) {
        const propertyId = (payload.PropertyId || payload.propertyID || payload.propertyId || payload.hid || "").toString().replace("RG:", "");
        ...
        try {
            ...
            try {
                ...
                const enrichDeep = (obj: any): any => {
                    ...
                    if (Array.isArray(obj.rate) || Array.isArray(obj.rates) || (obj.RoomSelectionKey && (obj.price || obj.net))) {
                        const rates = Array.isArray(obj.rate) ? obj.rate : (Array.isArray(obj.rates) ? obj.rates : null);
                        if (rates) {
                ...
                console.warn("[RateGain] Pricing enrichment warning (non-fatal):", enrichErr?.message);
            }
