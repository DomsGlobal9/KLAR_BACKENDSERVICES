...
async function testUrlAndHeaders(baseUrl: string, headers: any) {
    console.log(`Testing ${baseUrl}/hms/v3/hotel/listing with ${Object.keys(headers)}`);
    try {
        const res = await axios.post(`${baseUrl}/hms/v3/hotel/listing`, {
            checkIn: "2026-05-10",
            checkOut: "2026-05-12",
            rooms: [{ adults: 2 }],
            currency: "INR",
...
async function runTests() {
    ...
    const urls = ["https://apitest-hms.tripjack.com", "https://apitest.tripjack.com"];
    
    const headerScenarios = [
        { apikey: key },
        { apikey: key, agencyId: agency }
    ];
