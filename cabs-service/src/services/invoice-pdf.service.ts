...
// //       console.log("23 invoice-pdf.service - compileInvoiceHtml - backendData:", JSON.stringify(backendData, null, 2));
...
//     private getLogoAsBase64(): string {
//         try {
//             const logoPath = "D:\\Doms_Global_Projects\\klar_b2b\\KLAR_BACKENDSERVICES\\cabs-service\\src\\assets\\images\\klar-travels-logo.png";
//             if (fs.existsSync(logoPath)) {
//                 const bitmap = fs.readFileSync(logoPath);
//                 return `data:image/png;base64,${Buffer.from(bitmap).toString('base64')}`;
//             }
...
//         // Safe token mappings linking directly to the backend object values
//         const mappings: { [key: string]: string } = {
//             "{{logoSrc}}": base64Logo,
//             "{{bookingId}}": String(order.bookingId || "N/A"),
//             "{{status}}": currentStatus,
...
//             // Dynamic Financial Breakdown Strings
...
//             "{{grossAmount}}": Number(order.amount || pricing.grossAmount || 0).toLocaleString("en-IN", { minimumFractionDigits: 2 }),
            
//             // Policy Framework Strings
//             "{{waitingTime}}": String(policies.waitingTime || "30 mins free waiting time"),
//             "{{meetAndGreet}}": String(policies.meetAndGreet?.description || "Driver meets with placard at Arrivals Gate"),
